...
class nc_camera:
    ...
    def saveSequence(self, encode, nbr_imgs):
        try:
            error = ncCamStartSaveAcquisition(self.ncCam, self.name.encode(), encode, nbr_imgs, self.comment.encode(), 1, 1)
            if (error):
                raise NuvuException(error)
        except NuvuException as nuvuException:
            self.errorHandling(nuvuException.value())
    ...
    def addRoi(self, x, y, roiWidth, roiHeight):
        try:
            error = ncCamAddRoi(self.ncCam, x, y, roiWidth, roiHeight)
            if (error):
                raise NuvuException(error)
        except NuvuException as nuvuException:
            self.errorHandling(nuvuException.value())
