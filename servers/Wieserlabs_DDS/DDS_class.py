...
class DDS_funcs():
    ...
    def __init__(self):
        ...
        # self.inst = rm.open_resource('ASRL3::INSTR')
    ...
    def CONFIGURE_ROUTING(self, channel, route, inv, BNC_channel):
        ...
        if BNC_channel == 'off':
            BNC_channel = bin(0).replace('b', '0')

        return self.inst.query(f'dcp {channel} wr:CFG_{route}=0b010_{inv}_000_{BNC_channel}')

    def ASF_amp(self, channel, amp, full_amp=10.5):
        """set amp from ASF.
            amp (int) in dBm.
            amp_step_size for making ramps (set to minimum)"""
        ASF_dec = int(10**((amp - full_amp)/20) * (2**14 - 1))
        amp2atw = bin(ASF_dec)
        amp_step_size = '00' #binary string
        return self.inst.query(f'dcp {channel} spi:asf=' + amp2atw + amp_step_size)
    ...
    def DDS_reset_channel(self, channel):
        return self.inst.query(f'dds {channel} reset')
    ...
    def single_tone(self, channel, freq, amp, phase):
        ...
        self.DDS_reset()
        self.DDS_reset_channel(channel)
        self.edit_amp(channel)
        self.set_param(channel, freq*1e6, amp, phase)
        self.update()
        self.start()
        #self.close()

        print(f"channel {channel} set!: {freq}, {amp}, {phase}")
    ...
    def enable_am(self, channel, modulationChannel=0, scaleFactor = 1, offset = 0):
        
        # enable parallel data port
        # self.DDS_reset_channel(channel)
        self.inst.query('dcp spi:CFR2=0b00000000_00000000_01010000')
        # self.inst.query('dcp spi:CFR2=0x01000019')
        # set scale factor S0: here S0 = 2^12 = 0x1000
        self.inst.query(f'dcp {channel} wr:AM_S{modulationChannel}={hex(int(scaleFactor*0x1000) & 0x3ffff)}')
        # set offset O0: here O0 = 0 
        self.inst.query(f'dcp {channel} wr:AM_O{modulationChannel}={hex(int(offset*0x400000) & 0xffffff)}') 
    ...
    # Added by Ale on 03/08/2023.
    def set_fm(self, channel, f_min, f_max, amp, route, BNC_channel, inv = 0):
        # The modulation input of the channel is set to be the same of the channel (0 modulated by AM0 and 1 modulated by AM1)
        mod_input = channel

        # Find minimum and maximum frequency tuning word, i.e. min/max frequencies of the modulation 
        min_freqtuningword = freq_to_word(f_min*1e6)  
        max_freqtuningword = freq_to_word(f_max*1e6)
        ...
        # set OSK bits (switch on/off RF using a digital pin), sinc filter and sine output
        self.OSK_gate(channel)
        # set amplitude using the ASF register of the DDS
        self.ASF_amp(channel, amp)
        ...
        # Last 8 bits: (1 bit: Matched latency enable)(1 bit: Data assembler hold last value)(1 bit: Sync timing validation disable)(1 bit: parallel data port enable)(4 bits: FM gain in binary)
        self.inst.query(f'dcp {channel} spi:CFR2=0b_00000001_00000000_00000000_11011111')
        # set scale factor S0
        self.inst.query(f'dcp {channel} wr:AM_S{mod_input}={S0}')
        # set channel offset O0: here O0 = 0 
        self.inst.query(f'dcp {channel} wr:AM_O{mod_input}=0') 
        # set global offset O:
        ...
        self.update()
        # route BNC input A to the OSK digital pin (to control gate using the BNC input)
        self.CONFIGURE_ROUTING(channel, route, inv, BNC_channel)

        self.start()
        #self.close()
        
        print(f'Frequency modulation set on channel {channel}:\n\t- A = {amp} dBm\n\t- f_min = {f_min} MHz\n\t- f_max = {f_max} MHz\n\t- Gate on BNC input: {BNC_channel}')

if __name__ == '__main__':



    ''' Hi! I disabled all the commands "self.close" inside the functions, because some functions had it, some didn't, and if you want to do 2 functions in a row, the second would fail because the first already closed the connection. Better to call close() only at the end (and with a try/finally block)'''
    ...
    try:
        ...
        width=25#MHz
        power=7#dBm
        DDS.single_tone(channel,freq, power, 0)
        # DDS.enable_am(channel, modulationChannel = 1, offset = 0.)
        DDS.single_tone(0,0,0, 0)
        DDS.set_fm(channel, freq-width, freq+width, power, route='OSK', BNC_channel = 'A')
        #DDS.single_tone(0,34, 3, 0)
    finally:
        DDS.close()
    #DDS.single_tone(0,29, 2, 0)
    # DDS.single_tone(1, 275, 9, 0)
    # DDS.reset()
    # DDS.gate(0, 5, 30e6, 0, 'OSK', 0, 'A')
    # DDS.trigger_amp_ramp(0, 30e6, 5, 0, 0, 5, 600)
    # DDS.set_fm(0, 85e6, 135e6, 3)
    # DDS.set_fm(channel = 0, f_min = 79, f_max = 81, amp = -8.5, route='OSK', BNC_channel = 'off')