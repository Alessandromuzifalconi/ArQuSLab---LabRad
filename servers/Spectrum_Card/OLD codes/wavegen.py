...
def single_tone(sample_count, sample_rate, frequency, amplitude=1.0, phase=0.0,
...
def multi_tone(sample_count, sample_rate, frequencies, amplitudes,
               phases, normalize_amplitudes=True, round_frequencies=True):
    """
...
def _chirp(times, starting_frequency, duration, ending_frequency, phase): #since the chirp function by scipy does not support numba, I write a chirp function
    phase = 2 * np.pi * ((starting_frequency + 0.5 * ((ending_frequency - starting_frequency)/duration) * times) * times)
    signal = np.sin(phase)
    return signal
...
def _chirp_pulse(times, starting_frequency, duration, ending_frequency, amplitude, phase):
    ...
    return amplitude * _chirp(times, starting_frequency, duration, ending_frequency, phase)
